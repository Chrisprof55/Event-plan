...
export default function NovoItemForm({
  ...
  const [newEventPax, setNewEventPax] = useState('');
  ...
  useEffect(() => {
    if (creatingNew) {
      setNewEventDate((prev) => prev || date);
    }
  }, [creatingNew, date]);
  ...
  const handleEventSelect = (value) => {
    ...
    if (value === NEW_EVENT_VALUE) {
      setCreatingNew(true);
      setNewEventDate(date || defaultDate);
      onEventChange?.(null);
      return;
    }
  ...
  const handleCreateEvent = async () => {
    ...
    setCreateError(null);
    try {
      const eventDateValue = newEventDate || date;
      ...
      setNewEventDate('');
      setNewEventPax('');
      if (eventDateValue) setDate(eventDateValue);
      onEventChange?.(newId);
    } catch (err) {
          ...
          {showNewEventForm && (
            <div className="space-y-2 rounded-xl border border-amber-300/80 bg-white/70 p-3">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                ...
                <label className="block">
                  <span className="mb-1 block text-xs font-medium text-slate-600">Pax</span>
                  <input
                    type="text"
                    inputMode="numeric"
                    pattern="[0-9]*"
                    value={newEventPax}
                    onChange={(e) => setNewEventPax(e.target.value.replace(/\D/g, ''))}
                    placeholder="120"
                    className={inputClassName}
                    disabled={creatingEvent}
                  />
